        ...
        public Func<Party, IEnumerable<string>> Choose
        {
            get
            {
                return (Party party) =>
                {
                    ...
                    Character animalHelper = party.GetActiveCharactersWithStatus(Status.Polymorphed).FirstOrDefault();
                    if (animalHelper != null && (EncounterSelector.rng.NextDouble() < .7))
                    {
                        messages.Add($"Luckily, {animalHelper.PossessiveLower} keen animal sense of smell gives you all enough warning to avoid the worst of the gas.");
                        animalHelper.AwardXP(20, party.Depth);
